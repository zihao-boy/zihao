(function(vc) {
    ...
    vc.extends({
        ...
        _initEvent: function() {
            vc.on('addWafArea', 'openAddWafAreaModal', function() {
                $that._listWafRuleGroups();
                $that._loadChinaAreas();
                $that._loadForeignAreas();
                $('#addWafAreaModel').modal('show');
            });
        },
        methods: {
            addWafAreaValidate() {
                return vc.validate.validate({
                    addWafAreaInfo: vc.component.addWafAreaInfo
                }, {
                    'addWafAreaInfo.typeCd': [{
                            limit: "required",
                            param: "",
                            errInfo: "类型不能为空"
                        },
            ...
            saveWafAreaInfo: function() {
                if (!vc.component.addWafAreaValidate()) {
                    vc.toast(vc.validate.errInfo);

                    return;
                }

                if($that.addWafAreaInfo.areaNames.length < 1){
                    vc.toast('请选择位置');
                    return ;
                }

                $that.addWafAreaInfo.areaName = $that.addWafAreaInfo.areaNames.join(',');

                //不提交数据将数据 回调给侦听处理
                if (vc.notNull($props.callBackListener)) {
                    vc.emit($props.callBackListener, $props.callBackFunction, vc.component.addWafAreaInfo);
                    $('#addWafAreaModel').modal('hide');
                    return;
                }

                vc.http.apiPost(
                    '/firewall/saveWafArea',
                    JSON.stringify(vc.component.addWafAreaInfo), {
                        emulateJSON: true
                    },
                    function(json, res) {
                        ...
                        if (_json.code == 0) {
                            //关闭model
                            $('#addWafAreaModel').modal('hide');
                            vc.component.clearAddWafAreaInfo();
            ...
            clearAddWafAreaInfo: function() {
                vc.component.addWafAreaInfo = {
                    typeCd: '',
                    ip: '',
                    scope:'*',
                    seq:'',
            ...
            _listWafRuleGroups: function () {
                ...
                vc.http.apiGet('/firewall/getWafRuleGroup',
                    ...
                    function (json, res) {
                        ...
                        vc.component.addWafAreaInfo.wafRuleGroups = _wafRuleGroupManageInfo.data;
            ...
            _loadChinaAreas:function(){
                $that.addWafAreaInfo.chinaAreas=[
                    '山东','河北','吉林', '黑龙江','辽宁','内蒙古','新疆','甘肃','宁夏','山西','陕西','河南','安徽','江苏','浙江','福建','广东',
                    '江西','海南','广西','贵州','湖南','湖北','四川','云南','西藏','青海','天津','上海','重庆','北京','台湾','香港','澳门',
                ]
            },
            _loadForeignAreas:function(){
                $that.addWafAreaInfo.foreignAreas=[
