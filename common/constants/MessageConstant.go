...
func (code Code) String() string {
	...
	case CODE_USER_PHONE_FAILUR:
		return "用户名错误，或账号被冻结"
	case CODE_USER_PHONE_DUPLICATE:
		return "该号码已注册"
	case CODE_USER_PASSWORD_FAILUR:
		return "密码错误"
	case CODE_USER_REGISTE_OK:
		return "注册成功"
	case CODE_USER_LOGIN_OK:
		return "恭喜，登录成功"
	case CODE_USER_NOT_ENABLED:
		return "改用户已被冻结，请联系系统管理员"

	case CODE_SYS_OK:
		return "恭喜，成功"
	case CODE_SYS_FAILUR:
		return "失败"
	case CODE_SYS_ERROR:
		return "服务器内部错误"
	case CODE_SYS_PARSE_PARAMS_ERROR:
		return "请求参数错误"

	case CODE_TOKEN_CREATE_FAILUR:
		return "创建令牌失败"
	...
	case CODE_TOKEN_INVALID:
		return "无效令牌"
	case CODE_TOKEN_CACHE_ERROR:
		return "令牌缓存错误"
	case CODE_TOKEN_NOT_MAPCLAIMS:
		return "令牌不是MapClaims类型"

	case CODE_PERMISSION_NIL:
		return "没有操作权限"
